...
class ConfusionMatrixMetrics(tf.keras.metrics.AUC):
    ...
    def __init__(self,
        ...
        if isinstance(self, (Precision, Recall, F1, Accuracy)):
    ...
    def get_accuracy(self):
        ...
        return tf.squeeze(tf.math.divide_no_nan(tf.math.add(tp, tn), tf.math.add(tf.math.add(tp, fp), tf.math.add(tn, fn))))
...
class Accuracy(ConfusionMatrixMetrics):
    """Accuracy metrics"""
    def result(self):
        accuracy = self.get_accuracy()[1:-1]
        if self.multi_label:
            accuracy = self.get_macro_average(accuracy)
        return accuracy
...
def metrics_builder(num_labels: int) -> List[tf.keras.metrics.Metric]:
    ...
    # metric_classes = [Precision, Recall, Accuracy, AveragedPrecision]
    # metric_names = ["precision", "recall", "accuracy", "averaged_precision"]
