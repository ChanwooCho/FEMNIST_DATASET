...
#############################################################################################################
def resnet34(input_shape: Tuple[int, int, int],
             num_classes: int,
             pretrained: bool,
             seed: int = 0):
    """
...
def create_mobilenet(input_shape: Tuple[int, int, int],
    ...
    x = RandomFlip()(img_input)
    x = Rescaling(scale=1 / 255.0)(x)
    x = ZeroPadding2D((2, 2), name="pooling_padding")(x)
    # x = Normalization(
    #     axis=-1, mean=IMAGENET_MEAN, variance=IMAGENET_VARIANCE)(x)
    
    # bottleneck
    x = models.mobilenet_v3.MobileNetV3Small(input_shape=(32, 32, 1), include_top=False, weights=None)(x)
...
def create_squeezenet(input_shape: Tuple[int, int, int],
    ...
    # bottleneck
    x = squeezenet.SqueezeNet(input_shape=input_shape, nb_classes=num_classes)(x)
...
def create_efficientnet(input_shape: Tuple[int, int, int],
    ...
    # bottleneck
    x = models.efficientnet.EfficientNetB0(input_shape=(32, 32, 1), include_top=False, weights=None)(x)
